...
#   SPACE      PAINT on / off. While on, moving the cursor lays colour and the
#              mouse does not grab. Right-click erases. Colour comes from the
#              hue/sat/val trackbars on the window.
#   UP / DOWN  brush size
#   x          erase paint under the cursor          X   erase all paint
#
#   DIRECT edits stay where you put them. You do not need a key for that.
#   g          MELT: let unbound direct edits relax away again (default off)
#   f          bind / release the marked edit — a bound edit ignores melt.
#              The marked edit is the one under the mouse while dragging, else
#              the most recent; it is boxed in the overlay, yellow when bound.
#   F          release every bound edit at once
...
#   b          bake: current state becomes the new anchor identity AND every
#              direct edit on screen is bound
#   r          reset to anchor
...
# ---------------------------------------------------------------- render from params
def render_from_params(ren, px, py, sigma, theta, freq, coeff, extra=None):
    ...
    if extra is not None:
        # .to() so a CPU-built paint field cannot break a CUDA render
        out = out + extra.to(out.device, out.dtype)
    return torch.sigmoid(out)


class Paint:
    """Hand-painted colour, stored as dabs rather than pixels.

    A dab is mathematically a ZERO-FREQUENCY Gabor packet: a Gaussian envelope
    with a real coefficient and no carrier. It is added into the same
    pre-sigmoid sum the model's packets go into, so paint and structure pass
    through one nonlinearity instead of being composited as separate layers --
    and, like a packet, a dab has a position and a sigma, so the DirectOp stack
    moves it. Paint you put on a cheek travels with the cheek.

    Kept as plain lists; tensors are rebuilt only when the set changes.
    """

    def __init__(self):
        self.x, self.y, self.s, self.rgb = [], [], [], []
        self._t = None

    def __len__(self):
        return len(self.x)

    def add(self, x, y, sig, rgb, min_step=0.5):
        # rate limit: a new dab only when the cursor has actually moved
        if self.x:
            d2 = (x - self.x[-1]) ** 2 + (y - self.y[-1]) ** 2
            if d2 < (min_step * sig) ** 2:
                return False
        self.x.append(float(x)); self.y.append(float(y))
        self.s.append(float(sig)); self.rgb.append(tuple(rgb))
        self._t = None
        return True

    def erase(self, x, y, r):
        keep = [i for i in range(len(self.x))
                if (self.x[i] - x) ** 2 + (self.y[i] - y) ** 2 > r * r]
        if len(keep) == len(self.x):
            return 0
        gone = len(self.x) - len(keep)
        self.x = [self.x[i] for i in keep]; self.y = [self.y[i] for i in keep]
        self.s = [self.s[i] for i in keep]; self.rgb = [self.rgb[i] for i in keep]
        self._t = None
        return gone
    ...
    def tensors(self):
        if self._t is None and self.x:
            self._t = (torch.tensor(self.x), torch.tensor(self.y),
                       torch.tensor(self.s), torch.tensor(self.rgb))
        return self._t


def paint_field(dabs, stack, size, amount=4.0, chunk=96):
    """(1,3,size,size) pre-sigmoid field from the dabs, after the edit stack."""
    t = dabs.tensors()
    if t is None:
        return None
    dx, dy, ds, col = t
    dx, dy, ds = apply_stack_dabs(stack, dx.clone(), dy.clone(), ds.clone())
    g = (torch.arange(size, dtype=torch.float32) + 0.5) / size
    Yg, Xg = torch.meshgrid(g, g, indexing="ij")     # row = y, col = x
    out = torch.zeros(1, 3, size, size)
    for i in range(0, len(dx), chunk):
        sl = slice(i, i + chunk)
        e = torch.exp(-(((Xg[None] - dx[sl].view(-1, 1, 1)) ** 2
                         + (Yg[None] - dy[sl].view(-1, 1, 1)) ** 2)
                        / (2 * ds[sl].view(-1, 1, 1) ** 2 + 1e-12)))
        out = out + (amount * (2 * col[sl].view(-1, 3, 1, 1) - 1)
                     * e[:, None]).sum(0, keepdim=True)
    return out
...
class DirectOp:
    ...
    def __init__(self, w, pivot, r=0.10):
        self.w = w                       # (N,) soft weights
        self.pivot = list(pivot)         # grab point, unit coords
        self.r = float(r)                # grab radius, for weighting paint dabs
        self.dx = 0.0
        self.dy = 0.0
        self.rot = 0.0                   # radians
        self.log_s = 0.0                 # log scale
        self.bound = False               # 'f' — spring-back skips this edit
...
def apply_stack_dabs(stack, dx, dy, ds):
    """The SE(2)+scale the packets get, applied to paint dabs.

    An op's weights were fixed at grab time and are indexed BY PACKET, so a dab
    cannot use them; its weight is recomputed from the op's grab radius. Stated
    plainly because it is an approximation: it drops the envelope-reach and
    amplitude terms of grab_weights, neither of which has a dab analogue. A dab
    therefore follows a drag by distance alone, where a packet also had to be
    loud and to actually paint the grab point."""
    for op in stack:
        d2 = (dx - op.pivot[0]) ** 2 + (dy - op.pivot[1]) ** 2
        w = torch.exp(-d2 / (2 * op.r ** 2))
        w = torch.where(w < 0.02, torch.zeros_like(w), w)
        if abs(op.rot) > 1e-9 or abs(op.log_s) > 1e-9:
            cx, cy = op.pivot
            rx, ry = dx - cx, dy - cy
            if abs(op.rot) > 1e-9:
                a = w * op.rot
                ca, sa = torch.cos(a), torch.sin(a)
                rx, ry = ca * rx - sa * ry, sa * rx + ca * ry
            if abs(op.log_s) > 1e-9:
                sc = torch.exp(w * op.log_s)
                rx, ry = rx * sc, ry * sc
                ds = ds * sc
            dx, dy = cx + rx, cy + ry
        dx = dx + w * op.dx
...
def live(args):
    ...
    melt = False          # direct edits STICK by default; 'g' makes them relax
    paint = Paint()
    painting = False      # SPACE toggles
    brush = 0.05          # dab sigma, unit coords; UP/DOWN arrows
    dragging = False
    cursor = (0.5, 0.5)
    writer = None

    st = {"z": z, "dragging": False, "cursor": cursor, "painting": False}

    def brush_rgb():
        """Colour from the H/S/V trackbars, as float RGB in 0..1."""
        try:
            h = cv2.getTrackbarPos("hue", win)
            sa = cv2.getTrackbarPos("sat", win)
            v = cv2.getTrackbarPos("val", win)
        except cv2.error:
            return (1.0, 1.0, 1.0)
        bgr = cv2.cvtColor(np.uint8([[[h, sa, v]]]), cv2.COLOR_HSV2BGR)[0, 0]
    ...
    def on_mouse(ev, x, y, flags, _):
        ...
        st["cursor"] = (u, vv)
        if st["painting"]:
            # while painting, the mouse lays dabs and does not grab anything
            if ev in (cv2.EVENT_LBUTTONDOWN, cv2.EVENT_MOUSEMOVE):
                paint.add(u, vv, max(brush, 1.5 / m.ren.H), brush_rgb())
            elif ev == cv2.EVENT_RBUTTONDOWN:
                paint.erase(u, vv, brush * 1.5)
            return
        if ev == cv2.EVENT_LBUTTONDOWN:
            ...
            if mode == "MANIFOLD":
                ...
                live_op = DirectOp(w, (u, vv), r=r_grab)
    ...
    try:                       # not every highgui backend has trackbars;
        cv2.createTrackbar("hue", win, 0, 179, lambda _v: None)
        cv2.createTrackbar("sat", win, 0, 255, lambda _v: None)
        cv2.createTrackbar("val", win, 255, 255, lambda _v: None)
    except cv2.error as e:     # brush_rgb() already falls back to white
        print(f"no colour trackbars on this build ({e.__class__.__name__}); "
              f"paint will be white")
    if args.record:
    ...
    while True:
        ...
        # Direct-edit relaxation, OFF by default.
        #
        # It used to be driven by 'spring' and nested inside the wobble-OFF
        # branch above, which meant one flag did two unrelated jobs: relax the
        # LATENT back to the anchor identity (good in MANIFOLD mode) and
        # dissolve GEOMETRY edits (fatal in DIRECT mode -- you could never
        # place anything, and no amount of baking helped because bake cleared
        # the stack too). They are separate now. 'spring' is latent-only.
        # 'g' turns geometry melt on if you actually want it.
        if melt and stack and not dragging:
            for op in stack:
                if op.bound:
                    continue
            ...
            stack[:] = [o for o in stack
                        if o.bound or abs(o.dx) + abs(o.dy) + abs(o.rot)
                        + abs(o.log_s) > 1e-3]
        ...
        pf = paint_field(paint, stack + ([live_op] if live_op else []),
                         m.ren.H)
        img = render_from_params(m.ren, pxE, pyE, sgE, thE, frE, cf, extra=pf)
        ...
        if overlay:
            ...
            if painting:
                br = brush_rgb()
                bgr = (int(br[2] * 255), int(br[1] * 255), int(br[0] * 255))
                cv2.circle(frame, (int(cu * D), int(cvv * D)),
                           max(2, int(brush * D)), bgr, 2)
                cv2.rectangle(frame, (D - 34, D - 34), (D - 8, D - 8), bgr, -1)
                cv2.rectangle(frame, (D - 34, D - 34), (D - 8, D - 8),
                              (255, 255, 255), 1)
            else:
                cv2.circle(frame, (int(cu * D), int(cvv * D)),
            ...
            sel = live_op if live_op is not None else (stack[-1] if stack else None)
            if sel is not None:
                cv2.drawMarker(frame, (int(sel.pivot[0] * D), int(sel.pivot[1] * D)),
                               (0, 255, 255) if sel.bound else (200, 200, 200),
                               cv2.MARKER_SQUARE, 14, 2)
            n_bound = sum(1 for o in stack if o.bound)
            hud = (f"{mode}  pins {len(pins)}  "
                   f"edits {len(stack)}{f'/{n_bound} bound' if n_bound else ''}  "
                   f"bands {bands_txt}  r {r_grab:.2f}  "
                   f"|z-z0| {float((z - z_anchor).norm()):.2f}  "
                   f"err {err:.1f}px  {fps:.0f} fps  "
                   f"{f'PAINT {len(paint)} b{brush:.03f}  ' if painting else ''}"
                   f"{'spring' if spring else ''}"
                   f"{' wobble' if wobble else ''}{' MELT' if melt else ''}")
        ...
        # waitKeyEx, not waitKey: the arrow keys do not fit in 8 bits, and
        # `waitKey(1) & 0xFF` silently turns UP into 0 on Windows.  ASCII keys
        # still come through the low byte, so every existing binding is
        # unchanged.
        keyx = cv2.waitKeyEx(1)
        key = (keyx & 0xFF) if keyx >= 0 else 255
        if keyx in (2490368, 65362, 16777235, 82):        # UP   -- win/gtk/qt
            brush = min(0.40, brush * 1.15)
        elif keyx in (2621440, 65364, 16777237, 84):      # DOWN
            brush = max(0.004, brush / 1.15)
        # Which edit do , . < > and f act on? The one under the mouse while a
        # drag is live, otherwise the most recent on the stack. Without this
        # fallback the rotate keys were dead the moment you released the
        # button, because LBUTTONUP moves live_op into stack and nulls it.
        op_t = live_op if live_op is not None else (stack[-1] if stack else None)
        if key == ord('q'):
            break
        elif key == ord('m'):
            ...
            pins.clear(); stack.clear(); paint.clear()
            live_pin = live_op = None
        elif key == ord('['):
            r_grab = max(0.03, r_grab / 1.25)
        elif key == ord(']'):
            r_grab = min(0.5, r_grab * 1.25)
        elif key == ord(',') and op_t is not None:
            op_t.rot -= 0.08
        elif key == ord('.') and op_t is not None:
            op_t.rot += 0.08
        elif key == ord('<') and op_t is not None:
        ...
        elif key == ord('f') and op_t is not None:
            op_t.bound = not op_t.bound      # bind / release this edit
        elif key == ord('F'):
            for o in stack:
                o.bound = False              # release every bound edit
        elif key == ord(' '):
            painting = not painting
            st["painting"] = painting
        elif key == ord('x'):
            paint.erase(*st["cursor"], brush * 1.5)
        elif key == ord('X'):
            paint.clear()
        elif key == ord('g'):
            melt = not melt               # let unbound direct edits relax away
        elif key == ord('s'):
            spring = not spring
        elif key == ord('w'):
            wobble = not wobble
        elif key == ord('b'):
            z_anchor = z.clone()          # latent baseline
            for o in stack:
                o.bound = True            # and every edit on screen is kept
        elif key == ord('r'):
            z = z_anchor.clone(); v.zero_()
            stack.clear(); pins.clear(); paint.clear()
        elif key == ord('n'):
            z_anchor = torch.randn(128) * 0.8
            z = z_anchor.clone(); v.zero_()
            stack.clear(); pins.clear(); paint.clear()
