...
#      epoch — that was the real bottleneck, not the GPU. Every dataset
#      gets its own folder under --out, named after the --data_dir
#      folder itself: --data_dir faces1 -> ./runs/faces1/ holding
#      faces_cache_128.npy, model2.pt, loss.csv, and the recon/sample
#      preview PNGs, all in one place. --data_dir faces -> ./runs/faces/
#      instead — two different --data_dir folders can never collide on
#      the same cache or checkpoint again. Point back at a folder
#      you've already cached and training resumes from that cache in
#      seconds; --export needs the same --data_dir used for training so
#      it finds the matching run folder.
...
def run_dir(args):
    """Everything for one dataset lives under out/<dataset-folder-name>/ —
    the cache .npy, model2.pt, loss.csv, and the recon/sample preview
    images all land in the same place. Point --data_dir at a different
    folder and you get a different, untouched run_dir; point it back at
    a folder you've already cached and training resumes from that cache
    in seconds. --export must be run with the same --data_dir used for
    training, so it looks in the same run_dir to find model2.pt."""
    dataset_name = os.path.basename(os.path.normpath(args.data_dir))
    d = os.path.join(args.out, dataset_name)
...
def train(args, dev):
    rd = run_dir(args)
    cache = os.path.join(rd, f"faces_cache_{args.image_size}.npy")
    ...
    logf = open(os.path.join(rd, "loss.csv"), "a")
        ...
        if step % args.log_every == 0 or step == args.steps:
            ...
            with torch.no_grad():
                torch.save({"sd": model.state_dict(),
                           ...
                           os.path.join(rd, "model2.pt"))
                fx = batch_from(data, fixed_idx, dev)
                mu, _ = model.enc(fx)
                rc = model.ren(model.dec(mu))
                grid(torch.cat([fx, rc], 0),
                     os.path.join(rd, f"recon_{step:06d}.png"))
                grid(model.ren(model.dec(z_fixed)),
                     os.path.join(rd, f"sample_{step:06d}.png"))
            model.train()
    print("done ->", os.path.join(rd, "model2.pt"),
          " | now: python splat_trainer3.py --data_dir", args.data_dir, "--export")
...
def export(args, dev):
    rd = run_dir(args)                    # same --data_dir as training -> same run_dir
    ck = torch.load(os.path.join(rd, "model2.pt"), map_location="cpu")
...
def smoke():
    ...
    ck = torch.load(os.path.join(tmp, os.path.basename(os.path.normpath(imdir)),
                                 "model2.pt"), map_location="cpu")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default="./runs",
                    help="base dir; each dataset gets its own out/<folder-name>/ "
                         "subfolder holding its cache, checkpoint, and previews")
