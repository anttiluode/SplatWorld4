...
def render_from_params(ren, px, py, sigma, theta, freq, coeff, extra=None):
    ...
    renderer's own path (parity certified in SR0a).  If `extra` is provided
    it is added to the pre-sigmoid sum (e.g. paint field)."""
    out = None
...
# ================================================================ PAINT SYSTEM
class Paint:
    """A collection of soft Gaussian dabs (zero-frequency Gabor packets).
    Each dab has (x, y, sigma, r, g, b) in unit coordinates and RGB 0..1.
    They are stored in a list; transforms are applied when building the field."""

    def __init__(self):
        self.dabs = []

    def add(self, x, y, sigma, rgb):
        r, g, b = rgb
        self.dabs.append((float(x), float(y), float(sigma), float(r), float(g), float(b)))

    def erase(self, x, y, radius):
        """Remove all dabs within `radius` of (x,y) (unit coords)."""
        self.dabs = [d for d in self.dabs
                     if (d[0] - x) ** 2 + (d[1] - y) ** 2 > radius ** 2]

    def clear(self):
        self.dabs.clear()

    def __len__(self):
        return len(self.dabs)


def apply_directops_to_dab(op, x, y, sigma):
    """Apply one DirectOp to a single dab position and sigma.
    Rotation and scale preserve the dab's Gaussian shape."""
    if abs(op.rot) > 1e-9:
        cx, cy = op.pivot
        rx, ry = x - cx, y - cy
        ca, sa = math.cos(op.rot), math.sin(op.rot)
        x, y = cx + ca * rx - sa * ry, cy + sa * rx + ca * ry
        # sigma unchanged by rotation
    if abs(op.log_s) > 1e-9:
        s = math.exp(op.log_s)
        x = cx + (x - cx) * s  # scale about pivot
        y = cy + (y - cy) * s
        sigma *= s
    # translation
    x += op.dx
...
def paint_field(paint, ops, H):
    """Build the pre-sigmoid paint contribution as a (1,3,H,H) tensor.
    Each dab is a Gaussian envelope multiplied by RGB, summed.
    The `ops` list (DirectOp) is applied to each dab in sequence (the same
    operations that affect packet positions)."""
    if not paint.dabs:
        return None
    # apply all ops to each dab
    dabs = paint.dabs
    for op in ops:
        new_dabs = []
        for x, y, sigma, r, g, b in dabs:
            x, y, sigma = apply_directops_to_dab(op, x, y, sigma)
            new_dabs.append((x, y, sigma, r, g, b))
        dabs = new_dabs
    # build field on a grid (we'll accumulate per channel)
    # We'll create a 1x3xHxH tensor and fill using vectorised Gaussian.
    # For speed we loop over dabs, but number of dabs is small.
    field = torch.zeros((1, 3, H, H), dtype=torch.float32)
    # coordinates
    coords = torch.stack(torch.meshgrid(torch.linspace(0, 1, H),
                                        torch.linspace(0, 1, H), indexing='ij'), dim=-1)  # (H,H,2)
    # flatten to (H*H,2)
    grid = coords.reshape(-1, 2).to(field.device)
    for x, y, sigma, r, g, b in dabs:
        sigma = max(sigma, 1.0 / H)  # avoid tiny sigma causing numeric issues
        d2 = (grid[:, 0] - x) ** 2 + (grid[:, 1] - y) ** 2
        env = torch.exp(-d2 / (2 * sigma * sigma + 1e-12))
        env = env.reshape(H, H)
        field[0, 0] += r * env
        field[0, 1] += g * env
        field[0, 2] += b * env
...
class DirectOp:
    ...
    def __init__(self, w, pivot, r=None):
        ...
        self.r = r                       # grab radius, stored for info
    ...
    def on_mouse(ev, x, y, flags, _):
        ...
        if st["painting"]:
            ...
            if ev in (cv2.EVENT_LBUTTONDOWN, cv2.EVENT_MOUSEMOVE):
                paint.add(u, vv, max(brush, 1.5 / m.ren.image_size), brush_rgb())
