...
#   3. VECTORIZED RENDERER. The per-channel python loop is now two einsums
#      per chunk (env*cos and env*sin are computed once, not three times).
...
class GaborRenderer(nn.Module):
    def __init__(self, image_size=96, num_packets=256, chunk=64, use_checkpoint=False):
        ...
        self.register_buffer("GX", gx[None, None].clone())
        self.register_buffer("GY", gy[None, None].clone())
    ...
    def _chunk(self, px, py, sigma, theta, freq, coeff):
        """Vectorized math: compatible with OpenCV 5 DNN ONNX graph parsing."""
        ...
        # --- REPLACEMENT FOR EINSUM COMPATIBILITY ---
        B, n, H, W = ec.shape
        
        # Flatten spatial dimensions: (B, n, H, W) -> (B, n, H*W)
        ec_flat = ec.view(B, n, H * W)
        es_flat = es.view(B, n, H * W)
        
        # Permute coefficients for matrix multiplication: (B, n, 3) -> (B, 3, n)
        a_perm = a.permute(0, 2, 1)
        b_perm = b.permute(0, 2, 1)
        
        # Batched Matrix Multiply: (B, 3, n) x (B, n, H*W) -> (B, 3, H*W)
        out_cos = torch.bmm(a_perm, ec_flat)
        out_sin = torch.bmm(b_perm, es_flat)
        
        # Reshape back to standard image tensor shape: (B, 3, H, W)
        return (out_cos - out_sin).view(B, 3, H, W)

    def forward(self, raw):
        raw = raw.float()                                    # fp32 always
        px, py, sigma, theta, freq, coeff = self.activate(raw)
        out = torch.zeros(raw.shape[0], 3, self.H, self.W, device=raw.device)
        for i in range(0, self.N, self.chunk):
            ...
            if self.use_checkpoint and self.training:
                ...
                out = out + checkpoint(self._chunk, *args, use_reentrant=False)
            else:
                out = out + self._chunk(*args)
...
def smoke():
    ...
    a = argparse.Namespace(
        ...
        log_every=30, resume="", checkpointing=False, onnx=os.path.join(tmp, "t.onnx"))
